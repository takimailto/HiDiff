...
class BasicBlock(nn.Module):
    def __init__(self, emb_channels, inplanes, planes):
        ...
        self.binary_3x3= nn.Conv2d(inplanes, planes, 3, padding=1)
...
class DownBlock(nn.Module):
    def __init__(self, emb_channels, inplanes, planes):
        ...
        for i in range(self.scalor):
            self.binary_3x3.append(nn.Conv2d(inplanes, inplanes, 3, stride=2, padding=1))
...
class UNetModel(nn.Module):
    def __init__(
        ...
        self.block1_conv = BasicBlock(emb_channels=time_embed_dim, inplanes=model_channels, planes=model_channels)
        ...
        self.block2_conv = BasicBlock(emb_channels=time_embed_dim, inplanes=model_channels, planes=model_channels)
        ...
        self.block3_conv = BasicBlock(emb_channels=time_embed_dim, inplanes=model_channels * 2, planes=model_channels * 2)
        ...
        self.block4_conv = BasicBlock(emb_channels=time_embed_dim, inplanes=model_channels * 2, planes=model_channels * 2)
        ...
        self.block5_conv = BasicBlock(emb_channels=time_embed_dim, inplanes=model_channels * 4, planes=model_channels * 4)
        ...
        # middle block
        self.middle_block1 = BasicBlock(emb_channels=time_embed_dim, inplanes=model_channels * 4, planes=model_channels * 4)
        self.cross1 = CrossAttentionLayer(condition_dim = condition_dim_list[3], embedding_dim=model_channels * 4, emb_chn=time_embed_dim)
        self.middle_block2 = BasicBlock(emb_channels=time_embed_dim, inplanes=model_channels * 4, planes=model_channels * 4)

        # upsample block 1
        self.up_block1_up = UpBlock(emb_channels=time_embed_dim, inplanes=model_channels * 4, planes=model_channels * 4)
        self.up_block1_conv = BasicBlock(emb_channels=time_embed_dim, inplanes=model_channels * 8, planes=model_channels * 4)

        # upsample block 2
        self.up_block2_up = UpBlock(emb_channels=time_embed_dim, inplanes=model_channels * 4, planes=model_channels * 2)
        self.up_block2_conv = BasicBlock(emb_channels=time_embed_dim, inplanes=model_channels * 4, planes=model_channels * 2)

        # upsample block 3
        self.up_block3_up = UpBlock(emb_channels=time_embed_dim, inplanes=model_channels * 2, planes=model_channels * 2)
        self.up_block3_conv = BasicBlock(emb_channels=time_embed_dim, inplanes=model_channels * 4, planes=model_channels * 2)

        # upsample block 4
        self.up_block4_up = UpBlock(emb_channels=time_embed_dim, inplanes=model_channels * 2, planes=model_channels * 1)
        self.up_block4_conv = BasicBlock(emb_channels=time_embed_dim, inplanes=model_channels * 2, planes=model_channels * 1)

        # upsample block 5
        self.up_block5_up = UpBlock(emb_channels=time_embed_dim, inplanes=model_channels * 1, planes=model_channels * 1)
        self.up_block5_conv = BasicBlock(emb_channels=time_embed_dim, inplanes=model_channels * 2, planes=model_channels * 1)
